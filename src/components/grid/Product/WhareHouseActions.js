...
const WhareHouseActions = () => {
  const { importWarehouse, exportWarehouse, diathesimotita } = useContext(
    ...
    for (let product of selectedProducts) {
      if (!product.MTRL) {
        showMessage({
          severity: "error",
          summary: "Σφάλμα",
          message: "Το προϊόν δεν έχει κωδικό MTRL. Δεν υπάρχει στο softone",
        });
      }
    }
    try {
...
const Template = ({ CATEGORY_NAME, NAME, availability, MTRL }) => {
  const { setExportWarehouse, setImportWarehouse, setDiathesimotita } =
  ...
  const ActionMessage = () => {
    if (available < value) {
      return (
        <Message
          severity="info"
          text="Eισαγωγή στην αποθήκη"
          className="wharehouse-msg my-2"
        />
      );
    }
  ...
  return (
    <div className="flex align-items-center justify-content-between">
      <div className="mb-3">
        ...
        <div className="">
          <label htmlFor="minmax-buttons" className="font-bold block ml-1 mb-2">
            Αλλαγή:
          </label>
          <InputNumber
