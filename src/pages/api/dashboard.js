...
export default async function handler(req, res) {
    ...
    if(action == 'getStats') {
        try {   
            ...
            //COUNT HOW MANY:
            let withImages = await mongoResults({images: {$exists: true, $ne: []}});
            let inSoftone = await mongoResults({MTRL: {$exists: true}});
            let inSkroutz = await mongoResults({isSkroutz: true});
            let activeProducts = await mongoResults({ISACTIVE: true});
            let haveDescriptions = await mongoResults({ DESCRIPTION: { $exists: true, $type: "string", $ne: "" } })
               //CREATER THE STAT:
