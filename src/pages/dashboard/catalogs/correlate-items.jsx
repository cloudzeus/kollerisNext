...
const StepCorrelateProducts = () => {
    const {headers, attributes, mongoKeys } = useSelector((state) => state.catalog)
    ...
    useEffect(() => {
        ...
        if(totalLength === headers.length) {
            setContinue(true)
        }
     
    }, [headers, mongoKeys, attributes])
...
const Template = ({header, field}) => {
   
    return (
        <div className='flex justify-content-between'>
            <div>
                <p className='font-bold text-lg'> {field}</p>
                <p className='text-sm'>sample text: {header}</p>
            </div>
        </div>
    )
}
